...
def finnXfraY(x,a,b,C):
    ans = f(x,a,b,C) - C/2
    if ans < 0 + err and ans > 0 - err: # er innenfor nøyaktighet
        return 999999
    else:
...
for i in range(generasjoner):
    ...
    for i in range(antall_forslag):
        forslag[i] = np.random.choice(beste) * np.random.uniform(0.95, 1.05) # lager ny generasjon med mutasjoner fra de beste av den forrige generasjonen
