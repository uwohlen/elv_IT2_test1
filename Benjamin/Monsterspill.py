...
class Gjenstander: #konstruktør for gjenstander
    def __init__(self,navn,pris,iq,flaks,bonus,skade):
        self.navn = navn
        self.pris = pris
        self.iq = iq
        self.flaks = flaks
...
glock = Gjenstander("[G]Glock9",50000,0,0,0,20)
ammo = Gjenstander("[A]Ammo",5000,0,0,0,0)
knuckle_busters = Gjenstander("[K]Knuckle-Busters",25000,0,0,0,5)
pigg_sko = Gjenstander("[P]Pigg-Sko",25000,0,0,0,5)
vitaminer = Gjenstander("[V]Vitaminer",2500,0,0,1,0)
super_vitaminer = Gjenstander("[S]Super-Vitaminer",25000,0,0,2,0)
iq_tabletter = Gjenstander("[I]IQ-Tabletter",10000,20,0,0,0)
guds_velsignelse = Gjenstander("[GV]Guds-Velsignelse",15000,0,2,0,0)

items = []
våpen = []
butikk = [glock,ammo,knuckle_busters,pigg_sko,vitaminer,super_vitaminer,iq_tabletter,guds_velsignelse]
bonus = 0
    ...
    while True:
        ...
        if valg == "g" and 'Glock9' in items and 'Ammo' not in items:
            ...
            if karakter == "1":
                ...
                if grevenskade > 7:
                    slow_type(f'Desverre slo greven og gjorde {greven.skade} på deg')
        elif valg == "s":
            if karakter == "1":
...
#legge til et interface med info om grevenog at du interacte med han merx